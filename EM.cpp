...
int main( int /*argc*/, char** /*argv*/ )
{
    ...
    int i, j;
    int nsamples = 100;
    Mat samples( nsamples, 1, CV_32FC2 );
    Mat labels;
    Mat img = Mat::zeros( Size( 500, 500 ), CV_8UC3 );
    Mat sample( 1, 2, CV_32FC1 );

    //samples = samples.reshape(2, 0);
    cout<<samples.size()<<" "<<samples.channels()<<endl;
    for( i = 0; i < N; i++ )
    {
        ...
        cout<<samples_part.size()<<" "<<samples_part.channels()<<endl;
    }
    ...
    /*for(int i=0;i<samples.rows;i++){
        for(int j=0;j<samples.cols;j++)cout<<samples.at<float>(i,j)<<" ";
            cout<<endl;
    }*/
    // cluster the data
